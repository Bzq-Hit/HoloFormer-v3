...
def train(holo, epoch, model, optim, device, save_dir, **kwargs):
    ...
    for step in pbar:
        ...
        if use_ref:
            ...
            pbar.set_description(
                    'mse: {:.4f} tv_a:{:.4f} tv_p:{:.4f} loss: {:.4f} Z:{:.3f}'.format(
                        mse_loss.item(),
                        tv_loss_amp.item(),
                        tv_loss_phase.item(), loss.item(),
                        1000 * pred_depth))

        if ((step+1) % eval_period) == 0:

            #save _psnr_p and loss       
            # with open(os.path.join(save_dir, 'log.txt'), 'a') as f:
            #     f.write(str(_psnr_p.item()))
            #     f.write(',')
            #     f.write(str(loss.item()))
            #     f.write('\n')

            # save y_hat
            netout = y_hat[0, 0, :, :].cpu().data.numpy()
            ...
            plt.imsave(os.path.join(save_dir, f'y_hat_{step+1}.png'), netout, cmap='gray')
            ...
            #save real and imag from pred_amp and pred_phase
            complex_map = torch.polar(pred_amp, pred_phase)
            real_map = complex_map.real
            imag_map = complex_map.imag
            netout = real_map[0, 0, :, :].cpu().data.numpy()
            netout = (netout - np.min(netout))/(np.max(netout)-np.min(netout))
            plt.imshow(netout, cmap='gray')
            plt.show()
            netout = netout * 255
            if not os.path.exists(args.save_dir):
                os.makedirs(args.save_dir)
            plt.imsave(os.path.join(save_dir, f'Real_{step+1}.png'), netout, cmap='gray')
            netout = imag_map[0, 0, :, :].cpu().data.numpy()
            netout = (netout - np.min(netout))/(np.max(netout)-np.min(netout))
            plt.imshow(netout, cmap='gray')
            plt.show()
            netout = netout * 255
            if not os.path.exists(args.save_dir):
                os.makedirs(args.save_dir)
            plt.imsave(os.path.join(save_dir, f'Imag_{step+1}.png'), netout, cmap='gray')
            print('\tSaved middle result - real and imag') 
...
if __name__ =="__main__":
    ...
    parser.add_argument('--model_config', default='configs/HoloFormer3.yaml', type=str)
    parser.add_argument('--task_config', default='configs/sample.yaml', type=str)
    ...
    print('holo max and min:', holo.max(), holo.min())
    ...
    model = Pipe(model_config).to(device)
    model.summary()
    ...
    model, pred_amp, pred_phase, pred_depth = train(torch.tensor(holo), epoch, model, optim, device, save_dir, scale=scale,eval_period=eval_period, use_ref= use_ref)
